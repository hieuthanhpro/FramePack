...
high_vram = free_mem_gb > 12  # Giảm ngưỡng cho RAM 24 GB
...
def worker(input_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, mp4_crf):
    ...
    stream.output_queue.append(('progress', (None, '', make_progress_bar_html(0, 'Starting ...'))))

    try:
        if not high_vram:
            unload_complete_models(text_encoder, text_encoder_2, image_encoder, vae, transformer)

        stream.output_queue.append(('progress', (None, '', make_progress_bar_html(0, 'Text encoding ...'))))
        if not high_vram:
        ...
        stream.output_queue.append(('progress', (None, '', make_progress_bar_html(0, 'Image processing ...'))))
        ...
        stream.output_queue.append(('progress', (None, '', make_progress_bar_html(0, 'VAE encoding ...'))))
        if not high_vram:
            load_model_as_complete(vae, target_device=gpu)

        start_latent = vae_encode(input_image_pt, vae)

        stream.output_queue.append(('progress', (None, '', make_progress_bar_html(0, 'CLIP Vision encoding ...'))))
        if not high_vram:
        ...
        stream.output_queue.append(('progress', (None, '', make_progress_bar_html(0, 'Start sampling ...'))))
        ...
        for latent_padding in latent_paddings:
            ...
            if stream.input_queue.peek() == 'end':
            ...
            def callback(d):
                ...
                stream.output_queue.append(('progress', (preview, desc, make_progress_bar_html(percentage, hint))))
                return
...
def generate_images(prompt, seeds_input, num_inference_steps, guidance_scale, image_width, image_height, save_dir):
    ...
    try:
        ...
        seed_list = seed_list[:2]  # Limit to 2 images for RAM efficiency
    except ValueError:
        ...
        with gr.Tab("Generate Video"):
            with gr.Row():
                with gr.Column():
                    ...
                    quick_prompts = [['The girl dances gracefully with clear, charming movements.'], ['A character performs simple body movements.']]
                    vid_examples = gr.Dataset(samples=quick_prompts, label="Example Prompts", components=[vid_prompt])
                    vid_examples.click(lambda x: x[0], inputs=[vid_examples], outputs=vid_prompt, queue=False)
                    
                    with gr.Row():
                        vid_start = gr.Button("Start Video Generation")
                        vid_end = gr.Button("Stop Video Generation", interactive=False)
                    
                    with gr.Group():
                        vid_teacache = gr.Checkbox(label="Use TeaCache", value=True, info="Faster but may reduce hand/foot quality")
                        vid_n_prompt = gr.Textbox(label="Negative Prompt", value="", visible=False)
                        vid_seed = gr.Number(label="Seed", value=31337, precision=0)
                        vid_length = gr.Slider(label="Video Length (seconds)", minimum=1, maximum=8, value=4, step=0.1, info="Max 8s to save resources")
                        vid_latent_size = gr.Slider(label="Latent Window Size", minimum=1, maximum=33, value=9, visible=False)
                        vid_steps = gr.Slider(label="Steps", minimum=10, maximum=30, value=20, step=1, info="Recommend 20")
                        ...
                        vid_gpu_mem = gr.Slider(label="GPU Memory Reservation (MB)", minimum=2000, maximum=10000, value=8000, step=100, info="Increase if OOM error")
                        vid_mp4_crf = gr.Slider(label="MP4 Compression", minimum=0, maximum=100, value=16, info="Lower = better quality")
                
                with gr.Column():
                    vid_output_video = gr.Video(label="Generated Video", autoplay=True, height=512)
