...
if HF_TOKEN:
    ...
    print("Warning: HF_TOKEN not found. Please set it in environment variables or enter manually.")
    login()  # Yêu cầu nhập token nếu không có

# Thiết lập thư mục cache Hugging Face
os.environ['HF_HOME'] = os.path.abspath(os.path.realpath(os.path.join(os.path.dirname(__file__), './hf_download')))

# Cấu hình DifussionHandler cho sinh ảnh
    ...
    try:
        ...
        for latent_padding in latent_paddings:
            ...
            if use_teacache:
                ...
                transformer.initialize_teacache_free()
            ...
            def callback(d):
                ...
                if stream.input() == 'end':
                    stream.output(('end', None))
                    raise KeyboardInterrupt('User terminated the task.')
                ...
                stream.output(('progress', (preview, desc, make_progress_bar(percentage, hint))))
                return
            ...
            stream.output_queue.append(('file', output_filename))
        ...
        stream.output_queue.append(('end', None))
    return None
...
def process_video(input_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_teacache, mp4_crf):
    ...
    if input_image is None:
        yield "Vui lòng tải lên hình ảnh!", None, None, "", "", False, True
        return

    yield None, None, None, "", "Starting video generation...", False, True
    ...
    while True:
        flag, data = stream.output_queue.pop(0)
        if flag == 'file':
            output_filename = data
            yield output_filename, None, None, "", "", True, False
        elif flag == 'progress':
            preview, desc, html = data
            yield None, preview, None, desc, html, False, True
        elif flag == 'end':
            yield output_filename, None, None, "", "Video generation completed!", True, False
            break

def end_process():
    stream.input_queue.put('end')
...
with gr.Blocks(css=css) as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("Generate Static Images"):
            with gr.Row():
                with gr.Column():
                    ...
                    img_steps = gr.Slider(label="Số bước suy luận", minimum=10, maximum=100, value=50, step=1)
                    img_guidance = gr.Slider(label="Guidance Scale", minimum=1.0, maximum=10.0, value=3.0, step=0.1)
                    img_width = gr.Slider(label="Chiều rộng ảnh", minimum=256, maximum=1024, value=640, step=8)
                    img_height = gr.Slider(label="Chiều cao ảnh", minimum=256, maximum=1024, value=640, step=8)
                ...
                with gr.Column():
                    ...
                    img_output_gallery = gr.Gallery(label="Ảnh được sinh ra", columns=3, height="auto")
        ...
        with gr.Tab("Generate Video"):
            with gr.Row():
                with gr.Column():
                    vid_image = gr.Image(sources='upload', type="numpy", label="Input Image", height=320)
                    vid_prompt = gr.Textbox(label="Prompt", value='')
                    quick_prompts = [['The girl dances gracefully...'], ['A character doing simple body movements.']]
                    vid_examples = gr.Dataset(samples=quick_prompts, label='Quick Prompts', components=[vid_prompt])
                    vid_examples.click(lambda x: x[0], inputs=[vid_examples], outputs=vid_prompt)
                    
                    with gr.Row():
                        vid_start = gr.Button("Start Generation")
                        vid_end = gr.Button("End Generation", interactive=False)
                    
                    with gr.Group():
                        vid_teacache = gr.Checkbox(label="Use TeaCache", value=True)
                        vid_n_prompt = gr.Textbox(label="Negative Prompt", value="", visible=False)
                        vid_seed = gr.Number(label="Seed", value=31337)
                        vid_length = gr.Slider(label="Video Length (Seconds)", minimum=1, maximum=20, value=5, step=0.1)
                        vid_latent_size = gr.Slider(label="Latent Window Size", minimum=1, maximum=33, value=9, visible=False)
                        vid_steps = gr.Slider(label="Steps", minimum=25, maximum=100, value=25, step=1)
                        ...
                        vid_gpu_mem = gr.Slider(label="GPU Memory Preservation (MB)", minimum=1000, maximum=16000, value=6000)
                        vid_mp4_crf = gr.Slider(label="MP4 Compression", minimum=0, maximum=100, value=16)
                
                with gr.Column():
                    vid_output_video = gr.Video(label="Generated Video")
                    vid_preview = gr.Image(label="Preview Frames", height=200, visible=False)
